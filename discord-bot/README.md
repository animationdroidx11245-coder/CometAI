...
4. Install dependencies:
   ...
   npm install
   ```
5. Copy `.env.example` to `.env` and fill in `DISCORD_TOKEN` (add `GUILD_ID` too if you want commands to appear instantly in one test server while developing). `DISCORD_CLIENT_ID` defaults to the value in `config/appConfig.json`.
...
3. In the Discloud panel, set the environment variable `DISCORD_TOKEN` under your app's settings — do not commit real tokens into the zip. `DISCORD_CLIENT_ID` already ships with a default in `config/appConfig.json`; only add it as an env var if you ever need to override it (e.g. reusing this code for a different bot application).
